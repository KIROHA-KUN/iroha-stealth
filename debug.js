...
COMPUTER = createCanvas(40 * 0.7, 40 * 0.7, (c, can) => {
    ...
    c.fs("#6b3cff");
    c.beginPath();
    c.arc(cx, cy, can.width / 2 - 1, 0, PI * 2);
    c.fill();

    // Cincin
    c.fs("#2d0d5d");
    c.beginPath();
    c.arc(cx, cy, can.width / 2 - 3, 0, PI * 2);
    c.fill();

    // Inti portal
    c.fs("#000");
    c.beginPath();
    c.arc(cx, cy, can.width / 2 - 6, 0, PI * 2);
    c.fill();
...
let IROHA = new Image();
IROHA.src = "images/iroha.png";
let FRAME_SIZE = 362; // ukuran satu frame di PNG
let DRAW_SIZE = 64;   // ukuran karakter saat ditampilkan
let legLength = 6;
...
renderCharacter = (
    ...
    wrap(() => {
        ...
        if (isPlayer) {
            context.translate(-(DRAW_SIZE / 2), -(DRAW_SIZE / 2));
        } else {
            context.translate(-(bodyWidth / 2), -(bodyHeight / 2));
        }

        if (
            isPlayer &&
            IROHA.complete &&
            IROHA.naturalWidth > 0
        ) {
...
class Exit {
    ...
    (row + 1) * 40 - height + 8
        ...
        let pulse = 1 + sin(G.clock * PI * 2) * 0.08;
    ...
    COMPUTER,
    -COMPUTER.width / 2,
    -COMPUTER.height / 2 + 16
);

        // Jika ingin meja tetap ada, buka komentar baris di bawah
